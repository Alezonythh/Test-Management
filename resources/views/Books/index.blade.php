                        ...
                        <tbody>
                            @foreach ($books as $book)
                                <tr
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if (Auth::check() && Auth::user()->role == 'admin')
                                            <form id="addToCartForm-{{ $book->id }}" action="{{ route('keranjang.add', $book->id) }}" method="POST" class="inline-block mr-2">
                                                @csrf
                                                <button type="submit"
                    ...
                    <div id="modal-{{ $book->id }}" tabindex="-1"
                        ...
                        <div
                            ...
                            <form action="{{ route('books.pinjam') }}" method="POST" class="mt-6 space-y-4">
                                ...
                                <div>
                                    <label for="tanggal_kembali" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Tanggal
                                        Kembali</label>
        ...
        @if (Auth::check() && Auth::user()->role == 'admin')
            <button onclick="toggleCart()"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="white" class="w-7 h-7">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13l-1.3 5.2a1 1 0 001 1.3h12.6a1 1 0 001-1.3L17 13M7 13l1.5-5h7L17 13M9 21a1 1 0 100-2 1 1 0 000 2zm8 0a1 1 0 100-2 1 1 0 000 2z" />
                </svg>
            </button>

            <!-- Cart Modal -->
            <div id="cartModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <div class="bg-white dark:bg-[#2C3262] text-gray-800 dark:text-gray-100 rounded-2xl w-[95%] sm:w-[460px] max-h-[85vh] shadow-2xl flex flex-col overflow-hidden">
                    <div class="flex justify-between items-center p-4 border-b border-gray-200 dark:border-white/20">
                        ...
                        <button onclick="toggleCart()" class="text-gray-400 hover:text-gray-600 dark:hover:text-white transition">×</button>
                    </div>
                        ...
                        @if (count($cart) > 0)
                            <form action="{{ route('keranjang.checkout') }}" method="POST" id="cartForm">
                                ...
                                <div class="mb-3 p-3 rounded-lg bg-yellow-50 border border-yellow-200 dark:bg-white/10 dark:border-white/20">
                                    <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-200">Nama Peminjam</label>
                                    <input type="text" id="global-borrower-name" placeholder="Nama Peminjam" required value="{{ Auth::user()->name ?? '' }}" class="w-full p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-yellow-400 dark:bg-transparent dark:text-white">
                                    <p class="mt-1 text-xs text-gray-600 dark:text-gray-300">Akan diterapkan ke semua item.</p>
                                </div>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                                    <div class="p-3 rounded-lg bg-blue-50 border border-blue-200 dark:bg-white/10 dark:border-white/20">
                                        <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-200">Tanggal Pinjam</label>
                                        <input type="date" id="global-borrow-date" class="w-full p-2 border rounded-md dark:bg-transparent dark:text-white" value="{{ now()->format('Y-m-d') }}">
                                    </div>
                                    <div class="p-3 rounded-lg bg-blue-50 border border-blue-200 dark:bg-white/10 dark:border-white/20">
                                        <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-200">Tanggal Kembali</label>
                                        <input type="date" id="global-return-date" class="w-full p-2 border rounded-md dark:bg-transparent dark:text-white" value="{{ now()->addDays(7)->format('Y-m-d') }}">
                                    </div>
                                </div>

                                @foreach ($cart as $id => $item)
                                    <div class="p-3 rounded-lg border border-gray-200 dark:border-white/20 flex items-start justify-between gap-3">
                                        <div class="flex-1">
                                            <div class="font-semibold">{{ $item['judul_buku'] }}</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-300">Kategori: {{ $item['kategori'] }}</div>
                                            <input type="hidden" name="cart[{{ $id }}][nama_peminjam]" class="cart-item-borrower-name">
                                            <input type="hidden" name="cart[{{ $id }}][tanggal_pinjam]" class="cart-item-tanggal-pinjam">
                                            <input type="hidden" name="cart[{{ $id }}][tanggal_kembali]" class="cart-item-tanggal-kembali">
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <button type="button" onclick="updateQty(this, -1, {{ $id }})" class="px-2 py-1 rounded bg-gray-200 dark:bg-white/10">-</button>
                                            <input type="number" min="1" value="{{ $item['quantity'] }}" data-max="{{ $item['jumlah_stok'] }}" class="w-14 text-center p-1 rounded border dark:bg-transparent dark:text-white" name="cart[{{ $id }}][quantity]">
                                            <button type="button" onclick="updateQty(this, 1, {{ $id }})" class="px-2 py-1 rounded bg-gray-200 dark:bg-white/10">+</button>
                                        </div>
                                        <button type="button" onclick="removeFromCart({{ $id }})" class="text-red-600 hover:underline">Hapus</button>
                                    </div>
                                @endforeach

                                <div class="flex justify-end gap-3 mt-4">
                                    <button type="button" onclick="toggleCart()" class="px-4 py-2 rounded-lg border">Tutup</button>
                                    <button type="submit" class="px-4 py-2 rounded-lg text-white bg-green-600 hover:bg-green-700">Pinjam Semua</button>
                                </div>
                            </form>
                        @else
