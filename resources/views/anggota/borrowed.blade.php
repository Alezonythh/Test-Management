<x-app-layout>
    ...
    setTimeout(() => loading = false, 1500);" :class="open ? 'ml-64' : 'ml-16'"
        ...
        <div class="min-h-screen bg-gray-100 dark:bg-gray-900 py-6 px-4 sm:px-6 lg:px-8" x-show="!loading"
            ...
            <!-- Filter Tab -->
            <!-- Filter Tab -->
            <div class="mb-6 flex flex-col md:flex-row justify-center items-center gap-6">
                <form method="GET" action="{{ route('anggota.borrowedBooks') }}" class="flex space-x-6">
                    <!-- Tab Dipinjam -->
                    <label class="relative cursor-pointer group w-full md:w-auto">
                        <input type="radio" name="status" value="dipinjam" class="sr-only"
                            onchange="this.form.submit()" {{ $status == 'dipinjam' ? 'checked' : '' }}>
                        <span
                            class="block px-8 py-3 font-semibold rounded-3xl
